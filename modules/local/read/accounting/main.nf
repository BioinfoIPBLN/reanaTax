// Join fastp, HISAT2 and Kraken2 into a single per-sample read-fate table, and
// derive the host carry-over metric from it.
//
...
process READ_ACCOUNTING {
    ...
    input:
    tuple val(meta), path(fastp_json, stageAs: 'fastp/*'), path(hisat2_logs, stageAs: 'hisat2/*'), path(kraken2_reports, stageAs: 'kraken2/*')
    val host_taxid
