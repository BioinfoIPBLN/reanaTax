// Differential abundance with ANCOM-BC2, on the combined Bracken count table.
//
...
process DIFFERENTIAL_ABUNDANCE_ANCOMBC {
    tag "${meta.id}"
    label 'process_medium'

    conda "${moduleDir}/environment.yml"
    container "${workflow.containerEngine in ['singularity', 'apptainer'] && !task.ext.singularity_pull_docker_container
        ? 'https://depot.galaxyproject.org/singularity/bioconductor-ancombc:2.14.0--r45hdfd78af_0'
        : 'quay.io/biocontainers/bioconductor-ancombc:2.14.0--r45hdfd78af_0'}"
    ...
    tuple val("${task.process}"), val('ANCOMBC'), eval("Rscript -e 'cat(as.character(packageVersion(\"ANCOMBC\")))'"), emit: versions_r, topic: versions
    ...
    def prefix = task.ext.prefix ?: "${meta.id}"
    """
    touch ${prefix}.ancombc.results.tsv
    """
}
