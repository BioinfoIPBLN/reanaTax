...
def validateInputParameters() {
    ...
    if (params.run_taxpasta && (params.taxpasta_args ?: '') ==~ /.*--add-(name|rank|lineage|id-lineage|rank-lineage).*/ && !params.taxpasta_taxonomy_dir) {
        error("--taxpasta_args asks for taxon names, ranks or lineages, which Taxpasta reads from a taxdump: set --taxpasta_taxonomy_dir to a directory holding at least nodes.dmp and names.dmp.")
    }

    if (params.run_taxpasta && params.skip_kraken2 && !params.krakenuniq_db && !params.run_metaphlan) {
        log.warn("--run_taxpasta has nothing to standardise: it reads Kraken2, Bracken, KrakenUniq and MetaPhlAn output, and none of them is running. sylph, Metax and PathSeq write their own cohort tables.")
    }
